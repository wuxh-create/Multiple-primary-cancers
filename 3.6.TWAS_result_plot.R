...
rm(list=ls())
twas<-read.table("/home/wuxh/03.MultiPCancer/05.TWAS/03.result/merged_result.txt",header = T,sep = ",")


twas_filter<-twas[which(twas$gene != "gene"),]
...
#part1-------------------------------------------generate a data frame with genes and their counts-------------------------------------------------------------
{
  ...
  df_twas_score<-merge(twas_filter,df,by="gene_name")
  df_twas_score_select<-df_twas_score[,c(1,4,14)]
  
  #控制画图的顺序根据count值从大到小排序
  df_twas_score_select<-df_twas_score_select[order(df_twas_score_select$Count,decreasing = TRUE),]
  df_twas_score_select$Gene<-factor(df_twas_score_select$Gene,levels = df_twas_score_select$Gene)
  
  ggplot(df_twas_score_select, aes(x = Gene, y = Count,fill = Gene)) +
